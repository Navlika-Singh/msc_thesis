...
#rewards
def helpfulness_reward(completions, prompts=None, **kwargs):
    rewards = []
    judge_outputs = []
    ...
    if log_extra is not None:
        log_extra("judge_helpfulness_output", judge_outputs)

    return rewards

def safety_cost_reward(completions, prompts=None, **kwargs):
    rewards = []
    judge_outputs = []
    ...
    for i, completion in enumerate(completions):
        ...
        match = re.search(r'"Safety"\s*:\s*([+-]?\d)', raw_text)
    ...
    if log_extra is not None:
        log_extra("judge_safety_output", judge_outputs)

    return rewards

#main loop
if __name__ == "__main__":
    parser = TrlParser((ScriptArguments, CGRPOConfig, ModelConfig))
    ...
    #dataset
    datasets = [load_dataset("PKU-Alignment/BeaverTails-V", c)["train"] for c in HARM_CATEGORIES]
    ...
    trainer = CGRPOTrainer(
        ...
        args=training_args,
        reward_funcs=[helpfulness_reward, safety_cost_reward],
