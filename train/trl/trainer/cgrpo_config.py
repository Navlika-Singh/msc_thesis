...
class CGRPOConfig(_BaseConfig):
    ...
    cost_func_name: str | None = field(
        default=None,
        metadata={
            "help": "Name of the reward function (as it appears in `reward_func_names`) that represents the "
            "safety/cost signal for Constrained GRPO. If set, the trainer maintains a Lagrange multiplier that is "
            "dynamically updated via projected dual ascent to keep the expected cost near `safety_threshold`, "
            "instead of using a fixed weight for this reward function. Requires "
            "`multi_objective_aggregation='normalize_then_sum'`."
        },
    )
    reward_func: str | None = field(
        default=None,
        metadata={
            "help": "reward function dynamic vs static"
        },
    )
    safety_threshold: float = field(
        default=0.0,
        metadata={
            "help": "Target threshold d for the safety constraint (only used if `cost_func_name` is set): training "
            "pushes the expected cost J_C(pi) toward this value. Use 0.0 for a calibrated continuous cost model "
            "(negative=safe/positive=unsafe), or a small positive rate (e.g. 0.01) if the cost function is a 0/1 "
            "violation indicator."
        },
    )
    lambda_init: float = field(
        default=0.0,
        metadata={"help": "Initial value of the safety Lagrange multiplier lambda (only used if `cost_func_name` is set)."},
    )
    lambda_lr: float = field(
        default=0.01,
        metadata={
            "help": "Step size (eta) for the projected dual-ascent update of the safety Lagrange multiplier (only "
            "used if `cost_func_name` is set)."
        },
    )
    lambda_max: float | None = field(
        default=None,
        metadata={
            "help": "Optional upper bound (budget bound, nu_max) on the safety Lagrange multiplier, to prevent "
            "runaway growth and training instability/collapse, mirroring Safe RLHF-V's budget bound. `None` means "
            "unbounded (only used if `cost_func_name` is set)."
        },
    )
    ...
    def __post_init__(self):
        ...
        if self.cost_func_name is not None and self.multi_objective_aggregation != "normalize_then_sum":
            raise ValueError(
                "Constrained GRPO (`cost_func_name` set) requires `multi_objective_aggregation='normalize_then_sum'`. "
                "'sum_then_normalize' combines reward/cost before standardizing, which reintroduces the "
                "variance-distortion problem the scalarized-advantage formulation is designed to avoid (see "
                "Constrained GRPO, arXiv:2602.05863)."
            )
        if self.lambda_max is not None and self.lambda_max < self.lambda_init:
            raise ValueError(
                f"lambda_max ({self.lambda_max}) must be >= lambda_init ({self.lambda_init})."
            )
