...
class CGRPOTrainer(_BaseTrainer):
    """
    ...
    def __init__(
        ...
        #[UPDATE]
        # --- Constrained GRPO: Lagrange multiplier state for the safety constraint ---

        self.cost_func_idx = self.reward_func_names.index(args.cost_func_name) if args.cost_func_name else None
        self.reward_func = args.reward_func if args.reward_func else None
        # if self.cost_func_idx is not None and self.multi_objective_aggregation != "normalize_then_sum":
        #     raise ValueError(
        #         "Constrained GRPO (cost_func_name set) requires multi_objective_aggregation='normalize_then_sum'. "
        #         "'sum_then_normalize' combines reward/cost before standardizing and reintroduces the "
        #         "variance-distortion problem the scalarized-advantage formulation avoids."
        #     )
        self.safety_threshold = args.safety_threshold      # d
        self.lambda_lr = args.lambda_lr                    # eta
        self.lambda_max = args.lambda_max                  # nu_max, optional budget bound
        # self.lambda_safety = torch.tensor(float(args.lambda_init), dtype=torch.float32)  # lambda
        ...
        if resume:
            ...
            lambda_path = os.path.join(checkpoint_dir, "lambda_state.json") if checkpoint_dir else None
            print("[Lambda Path]", lambda_path)

            if lambda_path and os.path.exists(lambda_path):
                with open(lambda_path) as f:
                    saved_val = json.load(f)["lambda_safety"]
                self.lambda_safety = torch.tensor(saved_val, dtype=torch.float32)
                print(f"[Resume] lambda_safety restored: {self.lambda_safety.item():.6f}")
            else:
                self.lambda_safety = torch.tensor(float(args.lambda_init), dtype=torch.float32)
                print(f"[Resume failed set to init] lambda_safety restored: {self.lambda_safety.item():.6f}")
                print(torch.tensor(float(args.lambda_init), dtype=torch.float32))
        else:
            self.lambda_safety = torch.tensor(float(args.lambda_init), dtype=torch.float32)
            print(f"[No Resume] set to init: {self.lambda_safety.item():.6f}")

        if self.cost_func_idx is not None:
            self.reward_weights[self.cost_func_idx] = -self.lambda_safety
            if self.reward_func is not None and self.reward_func == "dynamic":
                self.reward_weights[0] = self.safety_threshold - self.lambda_safety #need to update, right now hard-coded assuming helpfulness reward is at '0' index
            elif self.reward_func is not None and self.reward_func == "dual":
                pass
            elif self.reward_func is not None and self.reward_func == "static":
                pass
    ...
    #[UPDATE]
    @torch.no_grad()
    def _update_lambda_safety(self, current_cost: float):
        """Projected dual-ascent update for the safety Lagrange multiplier (Eq. 3, Constrained GRPO)."""
        violation = current_cost - (-self.safety_threshold)         # J_C(pi) - d #ALERT! set safety_threshold to 3, added a negative sign here so the safety threshold right now is -3
        new_lambda = self.lambda_safety + self.lambda_lr * violation
        new_lambda = torch.clamp(
            new_lambda, min=1.0, max=self.lambda_max if self.lambda_max is not None else float("inf")
        )
        self.lambda_safety = new_lambda
        self.reward_weights[self.cost_func_idx] = -self.lambda_safety
        print("[DEBUG] inside update lambda safety", self.reward_func,)
        print("[DEBUG] violatiom", violation)
        print("[DEBUG] lambda safety", self.lambda_safety)
        print("[DEBUG] safety_threshold", self.safety_threshold)
        if self.reward_func is not None and self.reward_func == "dynamic":
            new_lambda_help = self.lambda_max - self.lambda_safety
            new_lambda_help = torch.clamp(
                new_lambda_help, min=1.0, max=self.lambda_max if self.lambda_max is not None else float("inf")
            )
            self.reward_weights[0] = new_lambda_help #need to update,
            self._metrics["train"]["lambda_helpfulness"].append(self.reward_weights[0].item())
        elif self.reward_func is not None and self.reward_func == "dual":
            print("[DEBUG] lambda dual")
            if violation < 0.0 and 1.0 <= self.lambda_safety < self.lambda_max:
                print("[DEBUG] lambda dual activated")
                new_lambda_help = self.lambda_max - self.lambda_safety
                new_lambda_help = torch.clamp(
                    new_lambda_help, min=1.0, max=self.lambda_max if self.lambda_max is not None else float("inf")
                )
                self.reward_weights[0] = new_lambda_help #need to update,
                self._metrics["train"]["lambda_helpfulness"].append(self.reward_weights[0].item())
                print("[DEBUG] lambda dual complete")
            print("[DEBUG] lambda dual complete")
        elif self.reward_func is not None and self.reward_func == "static":
            buffer = 5
            if violation < 0.0 and 1.0 <= self.lambda_safety < self.lambda_max - buffer:
                new_lambda_help = self.lambda_max
                self.reward_weights[0] = new_lambda_help #need to update,
                self._metrics["train"]["lambda_helpfulness"].append(self.reward_weights[0].item())
        print("[DEBUG] inside update lambda safety", self.reward_weights)
        self._metrics["train"]["lambda_safety"].append(self.lambda_safety.item())
    ...
    def _generate_and_score_completions(
        ...
        #     is_std_zero = torch.isclose(std_rewards, torch.zeros_like(std_rewards))  # for logging
            ...
            reward_k = (grouped - mean_k) / (std_k + 1e-4)
            reward_k = reward_k.view(-1, len(self.reward_funcs))
            # reward_weights here ARE the Lagrange multipliers when cost_func_idx is set:
            # reward_weights[reward_idx] = lambda_R, reward_weights[cost_func_idx] = -lambda_safety
            # safety_gate = torch.clamp(1.0 - reward_k[:, self.cost_func_idx], 0, 1)
            # reward_k[:, 0] = reward_k[:, 0] * safety_gate
            rewards = (reward_k * self.reward_weights.to(device).unsqueeze(0)).nansum(dim=1)
            rewards[unscorable_mask] = torch.nan

            if self.cost_func_idx is not None:
                # Constrained GRPO (Eq. 6): the scalarized advantage IS this weighted sum of standardized
                # components. No further outer standardization — that would distort the multiplier's effective
                # influence as lambda evolves over training, which the per-component standardization above was
                # specifically designed to prevent.
                advantages = torch.nan_to_num(rewards, nan=0.0)
                is_std_zero = torch.zeros_like(rewards, dtype=torch.bool)  # not meaningful here; kept for logging shape
            else:
                std_rewards = nanstd(rewards).expand_as(rewards) if rewards.numel() > 1 else torch.zeros_like(rewards)
                advantages = (rewards - torch.nanmean(rewards)) / (std_rewards + 1e-4)
        ...
        #[UPDATE]
        # Constrained GRPO: update lambda using this batch's observed cost. This runs AFTER `advantages` above was
        # already computed with the *previous* lambda — standard alternating dual-ascent ordering.
        if mode == "train" and self.cost_func_idx is not None:
            current_cost = torch.nanmean(rewards_per_func[:, self.cost_func_idx]).item()
            self._update_lambda_safety(current_cost)
    ...
    def _save_checkpoint(self, model, trial):
        if self.args.hub_model_id is None:
            ...
            model_name = self.args.hub_model_id.split("/")[-1]
        self.create_model_card(model_name=model_name)
        super()._save_checkpoint(model, trial)  # folder is created here

        # Save lambda after super() so checkpoint-{step}/ already exists
        with open(os.path.join(self.args.output_dir, "lambda_state.json"), "w") as f:
            json.dump({"lambda_safety": self.lambda_safety.item()}, f)
